...
const colorSize = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950]
    ...
    return (
        <main className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6">
            <div className="flex items-center">
                <h1 className="text-lg font-semibold md:text-2xl">{params.id}</h1>
            </div>
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <Select>
                                <SelectTrigger className="w-full">
                                    <SelectValue placeholder="Slate"/>
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                                        {colors.map((k: string, index: number) => {
                                            return <SelectItem value={k.toLowerCase()} key={index}>{k}</SelectItem>
                                        })}
                                    </SelectGroup>
                                </SelectContent>
                            </Select>
                            <Select>
                                ...
                                <SelectContent>
                                    <SelectGroup>
                                        {colorSize.map((k: number, index: number) => {
                                            return <SelectItem value={`${k}`} key={index}>{k}</SelectItem>
                                        })}
                                    </SelectGroup>
                                </SelectContent>
                            </Select>
